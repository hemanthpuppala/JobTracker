      ...
      <div className="overflow-y-auto flex-1">
        ...
        <Accordion title="Technical Skills" badge={store.skills.filter(s => s.included).length}>
          <div className="space-y-2">
            {store.skills.map(sk => (
              <div key={sk.id} className="flex items-start gap-2 group">
                ...
                <button
                  ...
                  className="text-red/60 hover:text-red text-xs bg-transparent border-none cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity mt-1.5"
                  title="Remove"
                >
                  &#10005;
                </button>
          ...
          <div className="space-y-1 mt-1">
            {exp.bullets.map((b, i) => (
              <div key={i} className="flex items-start gap-1 group/bullet">
                ...
                <textarea
                  className={`${inputCls} min-h-[28px] resize-y text-[0.7rem] leading-snug py-1`}
                  value={b}
                  rows={1}
